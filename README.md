...
4. 需要媒体库统计或保号检测时，填写 Emby 用户名和密码；系统会用账号密码自动登录 Emby，获取 AccessToken 和 UserId，用于读取资源数量和最后播放时间。
...
保号检测会复用同一套账号登录结果读取最后播放时间。程序会优先尝试 User Usage Stats / EmbyBoss 风格的播放记录接口；如果插件接口因权限不足、未安装或服务器错误不可用，会自动降级到标准 Emby `/Items`、Playback Reporting 和 ActivityLog 等其它来源。
...
- Emby 用户名和密码只用于后端自动登录获取 AccessToken/UserId，不需要也不会要求用户填写管理员 API Key。
- AccessToken 不会暴露在前端页面里，保存和请求都在 Worker/Docker 后端逻辑中完成。
- User Usage Stats 插件接口可能因权限不足不可用，程序会自动降级到其它最后播放来源。
