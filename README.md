...
一个部署在 Cloudflare Workers 上的 Emby 节点监控面板。它会定时探测多个 Emby 服务器的在线状态、延迟和最近 60 次历史记录，并支持 Telegram 通知、媒体库数量统计和自定义图标库。

## 功能

- 多节点在线状态监控，支持手动测速和定时测速。
- 近 60 次探测历史，可查看每个节点的可用率和离线次数。
- Telegram 通知，节点连续离线满 5 分钟后通知，恢复在线后通知。
- 媒体库资源统计，可记录电影、剧集、集数和 24 小时变化。
